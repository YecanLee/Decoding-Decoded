...
def extract_model_info(folder_path):
    ...
    model_name = parts[-2].split('-')[0]  # Extracts 'Llama3' from 'Llama-3_1-topk'
...
def log_metrics_results(folder_path, save_path):
    ...
    coherence_pattern = r"(.+)_(.+)_temp_(.+)_opt-2\.7b_coherence_result\.json"
    diversity_pattern = r"(.+)_(.+)_temp_(.+)_diversity_mauve_gen_length_result\.json"
    
    results = {}

    with open(output_file, 'w') as log_file:
        log_file.write("Dataset | Model | Decoding_Strategy | Temperature | Coherence Mean | Prediction Diversity Mean | MAUVE Mean\n")
        log_file.write("-" * 140 + "\n")
