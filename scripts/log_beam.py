...
def log_metrics_results(folder_path, save_path):
    ...
    coherence_pattern = r"(.+)_(.+)_beam_(.+)_opt-2\.7b_coherence_result\.json"
    diversity_pattern = r"(.+)_(.+)_beam_(.+)_diversity_mauve_gen_length_result\.json"
    
    results = {}

    with open(output_file, 'w') as log_file:
        log_file.write("Dataset | Model | Decoding_Strategy | Beam-Width | Coherence Mean | Prediction Diversity Mean | MAUVE Mean\n")
        log_file.write("-" * 140 + "\n")
